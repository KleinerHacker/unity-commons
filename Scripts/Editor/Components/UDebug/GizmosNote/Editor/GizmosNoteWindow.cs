...
namespace UnityCommons.Editor.Projects.unity_commons.Scripts.Editor.Components.UDebug.GizmosNote.Editor
{
    public sealed class GizmosNoteWindow : EditorWindow
    {
        ...
        private readonly SortedDictionary<string, IList<UnityCommons.Runtime.Projects.unity_commons.Scripts.Runtime.Components.UDebug.GizmosNote.GizmosNote>> _notes = new(StringComparer.Ordinal);
        ...
        private void ReloadGizmosNotes()
        {
            ...
            for (var i = 0; i < SceneManager.sceneCount; i++)
            {
                ...
                {
                    foreach (var gizmosNote in rootGameObject.FindComponents<UnityCommons.Runtime.Projects.unity_commons.Scripts.Runtime.Components.UDebug.GizmosNote.GizmosNote>())
                    {
                        if (!_notes.ContainsKey(gizmosNote.Category))
                        {
                            _notes.Add(gizmosNote.Category, new List<UnityCommons.Runtime.Projects.unity_commons.Scripts.Runtime.Components.UDebug.GizmosNote.GizmosNote>());
                        }
